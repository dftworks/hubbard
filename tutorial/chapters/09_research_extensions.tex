...
\item \textbf{General two-body tensors.}  Now supported by
      Chapter~\ref{chap:orbitalrotations}, with an explicit index and
      operator-ordering convention plus complete-spectrum invariance tests.
\item \textbf{Wavelet and learned rotations.}  Use the exact transformation as
      a reference, then transform both one- and two-body
...
\item Repeat the full-spectrum rotation test with a structured transform such
      as a discrete Fourier or wavelet matrix.  Compare tensor sparsity.
\end{enumerate}

The last exercise is a natural bridge from this tutorial to serious orbital
representation research: it tests not only code, but how representation
structure changes computational cost while preserving the physics.
