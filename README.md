...
The chaptered LaTeX tutorial in [tutorial/](tutorial/) derives the solver
incrementally and includes eight runnable programs directly in the manuscript.
It progresses from fixed-sector bases and fermionic signs through CSR and
matrix-free Hamiltonians, Lanczos residuals, observables, analytic validation,
complex generic hopping matrices, general interaction tensors, and exact
finite-`U` orbital rotations.  It verifies that rotating both terms preserves
the complete small-system spectrum and explains why rotating only the one-body
matrix is insufficient.

See [tutorial/README.md](tutorial/README.md) for code and PDF build commands.
...
Rotating the single-particle basis also transforms the onsite Hubbard
interaction into a general four-index two-body tensor.  Passing only a rotated
`h` while retaining onsite `U` therefore describes a different interacting
model, except at `U=0` or when the rotation preserves the local interaction.
The following tensor and rotation interfaces provide the exact finite-`U`
path.
...
## Exact orbital rotations

The rotation convention is `c_i = sum_a R[i,a] d_a`: columns of the unitary
matrix `R` are new orbitals expressed in the old basis.  Consequently,

\[
h' = R^\dagger h R,
\qquad
V'_{abcd}=\sum_{ijkl}R^*_{ia}R_{jb}R^*_{kc}R_{ld}V_{ijkl}.
\]

`rotate_hubbard_integrals(h, U, R)` transforms the one-body matrix and local
Hubbard interaction together.  The lower-level `rotate_one_body`,
`rotate_interaction_tensor`, and `rotate_integrals` functions support general
validated inputs.  All return independent read-only arrays.

```python
rotated_h, rotated_v = rotate_hubbard_integrals(h, U=4.0, rotation=R)
rotated_model = TwoBodyTensorHamiltonian(basis, rotated_h, rotated_v)
```

Unitary rotations preserve the complete many-body spectrum when both terms are
transformed.  Note that the existing local observables applied directly to a
rotated state measure orbital occupations; physical site observables must also
be transformed or evaluated after transforming the state back.
...
observable sum rules, arbitrary real and complex hopping matrices, general
interaction tensors, exact orbital-rotation spectral invariance, and eigenpair
residuals.
...
python examples/orbital_rotation.py
```
...
A generic orbital rotation makes the local interaction tensor dense, raising
the attempted two-body work to roughly `dimension * L**4` per matrix-vector
product.  For this pure-Python reference, rotated-tensor calculations are quick
at half-filled `L=4`, can be useful but slow at `L=6`, and exceed the default
5,000,000-term guard at `L=8`.  This smaller limit is separate from the sparse
site-basis limits above.
